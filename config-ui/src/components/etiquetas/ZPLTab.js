...
const ZPLTab = ({ isDarkMode }) => {
    ...
    const selectOutputDirectory = async () => {
        ...
        if (!ipcRenderer) {
            alert('Erro: Não foi possível conectar ao processo principal.');
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                addProcessingLog(`[ZPL] Diretório de saída selecionado: ${result.directory}`);
            }
        } catch (error) {
            console.error('Erro ao selecionar diretório:', error);
            alert('Erro ao selecionar diretório: ' + error.message);
        }
    };
    ...
    const handleFileSelect = (event) => {
        ...
        if (zplFiles.length > 0) {
            ...
            alert('Por favor, selecione apenas arquivos TXT ou ZPL.');
        }
    };
    ...
    const processSingleFile = async (fileItem) => {
        ...
        if (!ipcRenderer) {
            addProcessingLog('❌ Erro: Não foi possível conectar ao processo principal.', 'error');
        ...
        if (!outputDir) {
            alert('Por favor, selecione um diretório de saída antes de processar.');
            return false;
        }
        ...
        try {
            addProcessingLog(`[ZPL] Iniciando conversão de ${fileItem.file.name}...`);

            const zplContent = await fileItem.file.text();
            addProcessingLog(`[ZPL] Conteúdo lido (${zplContent.length} caracteres)`);
            ...
            const progressHandler = (event, progressData) => {
                ...
                setConversionStatus(progressData.status);
                addProcessingLog(`${fileItem.file.name}: ${progressData.status}`);
            };
            ipcRenderer.on('zpl-progress', progressHandler);

            const result = await ipcRenderer.invoke('convert-zpl', {
                zplContent,
                outputDir,
            ...
            // Remover listener
            ipcRenderer.removeListener('zpl-progress', progressHandler);
            ...
            if (result.success) {
                ...
                addProcessingLog(`[ZPL] ${fileItem.file.name}: Conversão concluída! ${result.totalEtiquetas} etiquetas processadas.`);
                addProcessingLog(`[ZPL] PDF salvo em: ${result.outputPath}`);

                // Remover arquivo da lista após processamento bem-sucedido
                setTimeout(() => {
    ...
    const processQueue = async () => {
        if (selectedFiles.length === 0) {
            alert('Por favor, selecione pelo menos um arquivo ZPL/TXT.');
            return;
        }
        ...
        setConversionStatus('Iniciando processamento da fila...');
        ...
        try {
            ...
            setConversionStatus('Fila de processamento concluída!');
        ...
        <div className="space-y-6">
            <div className="card">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <p className={`text-lg font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                                Arraste e solte arquivos ZPL/TXT aqui
                            </p>
                                ...
                                >
                                    <div className="flex items-center space-x-3 flex-1">
                                        <div className="text-2xl">
                                            {fileItem.status === 'processing' ? <Clock className="w-6 h-6 text-blue-500" /> : 
                                             fileItem.status === 'error' ? <XCircle className="w-6 h-6 text-red-500" /> : 
                                    ...
                                    {fileItem.status === 'pending' && !isConverting && (
                                        <button
                ...
                <div className="flex justify-end space-x-3">
                    ...
                    >
                        {isConverting ? '⏳ Processando...' : 
                         processMode === 'single' ? '✋ Processar Próximo' : '✋ Processar Todos'}
                    </button>
                </div>
            </div>

            {/* Painel de Progresso */}
            {(isConverting || processingLogs.length > 0) && (
                <div className="card">
                    <h3 className={`text-lg font-semibold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                        [PROGRESSO] Progresso da Conversão
                    </h3>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className={`p-6 rounded-lg shadow-xl max-w-md w-full mx-4 ${
                        ...
                        <div className="flex items-center mb-4">
                            ...
                            <h3 className="text-lg font-semibold">Erro na Conversão</h3>
                        </div>

                        <p className={`mb-6 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                            Ocorreu um erro ao converter o arquivo <strong>{errorFileName}</strong>.
                            Deseja continuar convertendo os próximos arquivos da fila?
                        </p>
